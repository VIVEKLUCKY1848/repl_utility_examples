...
::JREPL.BAT version 4.0 by Dave Benham
::
::  Release History:
...
if .%2 equ . (
  if "%~1" equ "/?" (
    ...
    call :version
    exit /b 0
  ) else call :exitErr "Insufficient arguments"
)
...
:help
call :GetScript   REM into /test variable
for /f "tokens=* delims=:" %%A in ('findstr "^:::" "%/test%"') do @echo(%%A
exit /b 0

:pagedHelp
call :GetScript   REM into /test variable
(for /f "tokens=* delims=:" %%A in ('findstr "^:::" "%/test%"') do @echo(%%A)|more /e
exit /b 0

:version
call :GetScript   REM into /test variable
for /f "tokens=* delims=:" %%A in ('findstr "^::JREPL\.BAT" "%/test%"') do @echo(%%A
exit /b 0
